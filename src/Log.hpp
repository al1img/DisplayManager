 ...
 * either instance of Log or string, second one is
 * LogLevel (for macro use DISABLE, ERROR, WARNING, INFO, DEBUG).
 * In case of Log instance all log parameters are kept inside
 ...
 * @param[in] instance log instance (Log) or <i>const char*</i> or
 *                         <i>nullptr</i>
 * @param[in] level    log level
 * @ingroup log
 */
#define LOG(instance, level)                        \
	LogLine().get(instance, __FILENAME__, __LINE__, \
				  LogLevel::log##level)
...
#define DLOG(instance, level) \
	true ? (void)0 : LogVoid() & LOG(instance, level)
...
public:
	...
	Log(const std::string &name, LogLevel level = Log::getLogLevel(),
		bool fileAndLine = Log::getShowFileAndLine()) : mName(name), mLevel(level), mFileAndLine(fileAndLine)
	{
		setLogLevelByMask();
	}
...
private:
									...
									LogLevel &level)
	{
		...
		{
			if (std::string(strLevelArray[i]).compare(0, strLevelUp.length(), strLevelUp) == 0)
			{
